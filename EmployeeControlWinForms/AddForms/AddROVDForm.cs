...
{
    public partial class AddROVDForm : Form
    {
        public int id;
        ...
        private void AddButton_Click(object sender, EventArgs e)
        {
            ...
            {
                query = "INSERT INTO [ROVD] (Id_City, name) VALUES (@value1, @value2)";
                uniquenessQuery = "SELECT COUNT(*) FROM ROVD WHERE (Id_City = @value1 AND name = @value2)";
            }
            else
            {
                query = "UPDATE ROVD SET Id_City=@value1, name=@value2 WHERE Id=@id";
                uniquenessQuery = $"SELECT COUNT(*) FROM ROVD WHERE (Id_City = @value1 AND name = @value2) AND id != '{id}'";
            }
